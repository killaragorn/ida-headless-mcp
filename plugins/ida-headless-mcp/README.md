...
codex plugin add ida-headless-mcp@ida-headless-mcp
```

Restart or refresh Codex if the new MCP server is not visible immediately. The marketplace entry points at `plugins/ida-headless-mcp/`, whose manifest declares an `ida-headless` stdio MCP server that runs `python ./scripts/launch.py --stdio`.
...
make plugin-sync    # Sync runtime into root and Codex marketplace plugin packages
make prebuilt       # Sync runtime, rebuild root bin/ plugin binaries, then sync marketplace package
...
├── plugins/ida-headless-mcp/
│   └── ...               # Codex marketplace-installable runtime package
