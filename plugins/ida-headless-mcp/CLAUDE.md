...
- License:MIT
- 仓库根目录保留直接安装用的插件包;Codex marketplace 安装包位于 `plugins/ida-headless-mcp/`;源码、测试和开发配置集中在 `src/`。
...
```
ida-headless-mcp/
├── .agents/plugins/marketplace.json # Codex marketplace 索引,指向 plugins/ida-headless-mcp
...
├── plugins/ida-headless-mcp/      # Codex marketplace 可安装的插件运行时包
...
源码改动涉及 Python worker 或插件运行时文件时,进入 `src/` 运行 `make plugin-sync` 或 `make prebuilt` 同步根目录运行时包和 `plugins/ida-headless-mcp/` Codex marketplace 包。
